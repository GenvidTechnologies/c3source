...
- `decisions/0022-domain-fact-audit-convention.md` — domain-fact tables carry a confidence label in JSDoc (`AUDITED`/`KNOWN INCOMPLETE`/`UNVALIDATED`/`NOT CORPUS-AUDITABLE`); the scanner reports partitions, never verdicts; numbers live in `docs/domain-fact-audit.md`, not JSDoc (#68)
