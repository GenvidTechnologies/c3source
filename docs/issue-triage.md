...
>
> This is the **flat-label variant**: c3source uses GitHub's default
> category-label set (`bug`, `enhancement`, `documentation`, `duplicate`,
> `question`, `wontfix`) plus `triaged`, with **no** `type:`/`priority/`/`area:`
> scheme. Tracker is GitHub Issues via the `gh` CLI. The section headings must
> stay as they are — the skill and its analyst locate guidance by heading.
