...
> **maintenance schema** for c3source's three-tier wiki: `raw/` (immutable
...
**This project's directories.** `.gvt-agent.json` sets `wiki.wikiDir` = `wiki`
and `wiki.rawDir` = `raw`. Wherever this document writes `<wikiDir>/` or
`<rawDir>/`, read them as `wiki/` and `raw/` respectively. The placeholders are
kept deliberately — this file stays diffable against the plugin's canonical
template, so a plugin update's schema changes are visible as real edits rather
than drowned in a name substitution.
...
**c3source's rule.** Volatility here is governed by one thing above all: C3
is an **undocumented, moving target**. Every page is sorted by what a new
Construct 3 release can invalidate.

- **C3 platform facts** — domain-fact tables, on-disk serialization form,
  release pins, editor-loader strictness. A C3 release can falsify these
  without notice, so they get a `stale_after` **six months** out. Re-validate
  against `editor.construct.net/r{NNN}/` or a corpus scan, not by re-reading
  the page.
- **Accepted decision records (ADRs)** — these state the situation *at their
  date* and are never retro-edited, so they **omit `stale_after` entirely**.
  An ADR does not go stale; it gets **superseded**, which is recorded by
  flipping `status` to `deprecated` and naming the superseding page.
- **Library API surface and architecture** — module DAG, exported functions,
  the `C3Project` handle. Changes on this repo's own release cadence, which is
  observable from `CHANGELOG.md`, so a stale date adds little: `stale_after`
  **one year** out.
- **Process and tooling practice** — fixture materialization, CI, publishing,
  triage conventions. Actively refined: `stale_after` **six months** out.

When a page mixes tiers, take the **shortest** of the applicable windows — a
page is only as fresh as its most volatile claim.
