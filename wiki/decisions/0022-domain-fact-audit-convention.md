...
title: "ADR 0022 — Domain-fact audit convention: confidence labels, evidence not verdicts"
description: Every domain-fact table carries a confidence label (AUDITED / KNOWN INCOMPLETE / UNVALIDATED / NOT CORPUS-AUDITABLE) paired with its blast radius; scripts/scan-domain-facts.mjs reports partitions for a human to judge rather than asserting a table is correct; numbers live in docs/domain-fact-audit.md, labels live in JSDoc.
tags: [adr, domain-facts, testing]
status: stable
generated: { by: process:maintain-wiki, at: 2026-08-20T15:48:57Z }
sources:
  - id: adr-0022
    resource: ../../raw/adr-0022-domain-fact-audit-convention-2026-08-20.md
    title: "ADR 0022 (docs/decisions capture, 2026-08-20)"
    last_modified: 2026-08-20
---

# ADR 0022 — Domain-fact audit convention: confidence labels, evidence not verdicts

**Status:** accepted
**Date:** 2026-08-04
**Issue:** #68

Migrated verbatim from the `docs/decisions/` ADR record[^adr-0022].

## Context

[ADR 0008](/decisions/0008-c3-domain-fact-tables.md) established C3 domain facts as
...
channel"](/c3-domain-facts.md#two-evidence-channels) section for the full detail.
...
- `docs/domain-fact-audit.md` is the numbers half of this convention; see
  [ADR 0023](/decisions/0023-pre-r402-image-serialization-drift-degradation.md) for the
  first table this audit found genuinely wrong, and the [ADR 0008
  amendment](/decisions/0008-c3-domain-fact-tables.md) for the full defect list.
...
experiment)"](/c3-domain-facts.md) section. Being right is not the same as
being evidenced, and nothing in this convention distinguished the two for
prose.
...
backed by [ADR 0024](/decisions/0024-script-source-fact-and-dotted-extensions.md) and
...
- [ADR 0008 — C3 domain facts owned as exported tables](/decisions/0008-c3-domain-fact-tables.md) — the record this ADR extends with a validation convention.
- [ADR 0021 — Reference integrity as a separate module](/decisions/0021-reference-integrity-detection.md) — the `C3_PSEUDO_OBJECT_CLASSES` seeding failures this ADR's Context cites as motivation.
- [ADR 0023 — Pre-r402 image serialization and drift degradation](/decisions/0023-pre-r402-image-serialization-drift-degradation.md) — the first table this audit found genuinely wrong.
- [ADR 0024 — Script-source domain fact and dotted-extension convention](/decisions/0024-script-source-fact-and-dotted-extensions.md) — the house-standard example this ADR's prose amendment cites.
- [C3 Domain Facts](/c3-domain-facts.md) — the current state of the audit convention this ADR establishes.

[^adr-0022]: ADR 0022 (docs/decisions capture, 2026-08-20)
