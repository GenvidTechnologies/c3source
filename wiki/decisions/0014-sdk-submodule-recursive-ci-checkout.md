...
title: "ADR 0014 — Keep the Construct Addon SDK submodule read-only; recursive CI checkout via the shared workflow's submodules input"
description: The Scirra Construct Addon SDK stays a read-only git submodule; CI checks it out recursively via a new optional submodules input added to the shared node-gate.yml reusable workflow, enabling the previously silently-skipped SDK-gated addon tests.
tags: [adr, ci, submodules, addons]
status: stable
generated: { by: process:maintain-wiki, at: 2026-08-20T15:48:57Z }
sources:
  - id: adr-0014
    resource: ../../raw/adr-0014-sdk-submodule-recursive-ci-checkout-2026-08-20.md
    title: "ADR 0014 (docs/decisions capture, 2026-08-20)"
    last_modified: 2026-08-20
---

# ADR 0014 — Keep the Construct Addon SDK submodule read-only; recursive CI checkout via the shared workflow's submodules input

**Status:** accepted (complements [ADR 0013](/decisions/0013-fflate-dependency-c3addon-reader.md))
**Date:** 2026-07-21
**Issue:** #49 (also references #50, #51)

Migrated verbatim from the `docs/decisions/` ADR record[^adr-0014].
...
submodule. [ADR 0013](/decisions/0013-fflate-dependency-c3addon-reader.md)'s
...
constants (cf. [ADR 0008](/decisions/0008-c3-domain-fact-tables.md)) — and
clones still need `--recursive` / a submodule init to exercise the tier
locally. The shared `node-gate` workflow now has a reusable `submodules`
input other consumers can adopt.

## Related

- [ADR 0013 — Depend on fflate for .c3addon zip reading](/decisions/0013-fflate-dependency-c3addon-reader.md) — the reader this SDK-gated tier exercises.
- [ADR 0015 — Adopt construct3-sample as the canonical C3 reference fixture](/decisions/0015-canonical-c3-reference-fixture.md) — the sibling fixture-ownership decision that followed one day later, complementing rather than replacing the SDK submodule.
- [ADR 0008 — C3 domain facts owned as exported tables](/decisions/0008-c3-domain-fact-tables.md) — why the shapes the SDK submodule pins are owned here as constants rather than re-verified per consumer.
- [Addon Domain Layer](/addon-domain-layer.md) — the reader/parser this SDK-gated tier tests.

[^adr-0014]: ADR 0014 (docs/decisions capture, 2026-08-20)
