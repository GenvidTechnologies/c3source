...
title: "ADR 0004 — Package entry points at dist/, not src/*.ts via publishConfig"
description: package.json's main/types/exports point directly at the built dist/*.js and dist/*.d.ts artifacts, never at src/*.ts rewritten via publishConfig, because npm (unlike pnpm/yarn) ignores that override and ships broken src/ paths.
tags: [adr, packaging, publishing]
status: stable
generated: { by: process:maintain-wiki, at: 2026-08-20T15:48:57Z }
sources:
  - id: adr-0004
    resource: ../../raw/adr-0004-dist-entry-points-no-publishconfig-2026-08-20.md
    title: "ADR 0004 (docs/decisions capture, 2026-08-20)"
    last_modified: 2026-08-20
---

# ADR 0004 — Package entry points at dist/, not src/*.ts via publishConfig

**Status:** accepted
**Date:** 2026-05-30
**Issue:** #8

Migrated verbatim from the `docs/decisions/` ADR record[^adr-0004].
...
`dist/` before any `npm pack`/`npm publish`. The old `publishConfig` src-rewrite trick is
...
`src/` directly ([ADR 0001](/decisions/0001-single-module-esm-library.md)).
...
[ADR 0003](/decisions/0003-github-actions-oidc-publishing.md).

## Related

- [ADR 0001 — Single-module ESM library](/decisions/0001-single-module-esm-library.md) — the ESM substrate; tests resolve `src/` via `tsx` while the package itself resolves `dist/`.
- [ADR 0003 — CI/publish via GitHub Actions + npm + OIDC trusted publishing](/decisions/0003-github-actions-oidc-publishing.md) — the npm move this decision fixes a consequence of.
- [CI & Publishing](/ci-and-publishing.md) — the current publish pipeline this entry-point convention supports.

[^adr-0004]: ADR 0004 (docs/decisions capture, 2026-08-20)
