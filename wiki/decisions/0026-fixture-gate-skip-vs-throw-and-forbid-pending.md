...
title: "ADR 0026 — Split the fixture gate into skip-if-absent / throw-if-moved, backed by a computed --forbid-pending"
description: fixtureProjectAvailable/sdkFixtureAvailable replace bare existsSync gates with a three-way answer (skip if the fixture root is absent, throw if a specific path is missing, else proceed), and a computed .mocharc.cjs arms --forbid-pending only when both gated fixtures are fully materialized.
tags: [adr, testing, fixtures]
status: stable
generated: { by: process:maintain-wiki, at: 2026-08-20T15:48:57Z }
sources:
  - id: adr-0026
    resource: ../../raw/adr-0026-fixture-gate-skip-vs-throw-and-forbid-pending-2026-08-20.md
    title: "ADR 0026 (docs/decisions capture, 2026-08-20)"
    last_modified: 2026-08-20
---

# ADR 0026 — Split the fixture gate into skip-if-absent / throw-if-moved, backed by a computed --forbid-pending

**Status:** accepted
**Date:** 2026-08-19
**Issue:** #82

Migrated verbatim from the `docs/decisions/` ADR record[^adr-0026].
...
designed to tolerate, per [ADR 0019](/decisions/0019-hermetic-fixture-materialization.md))
...
upstream-owned ([ADR 0015](/decisions/0015-canonical-c3-reference-fixture.md)) — a
fixture change belongs to a separate decision with its own tag and pin bump,
not folded into a test-infrastructure fix.
...
- [ADR 0013 — Depend on fflate for .c3addon zip reading](/decisions/0013-fflate-dependency-c3addon-reader.md) — unaffected by this decision, noted explicitly to rule out confusion.
- [ADR 0014 — SDK submodule read-only, recursive CI checkout](/decisions/0014-sdk-submodule-recursive-ci-checkout.md) — unaffected by this decision, noted explicitly to rule out confusion.
- [ADR 0015 — Adopt construct3-sample as the canonical C3 reference fixture](/decisions/0015-canonical-c3-reference-fixture.md) — extended, not revised, by this ADR.
- [ADR 0019 — Hermetic fixture materialization](/decisions/0019-hermetic-fixture-materialization.md) — the degradation contract this ADR protects with a second layer rather than changing.
- [Canonical Reference Fixture](/canonical-fixture.md) — the current state of the skip/throw split and `--forbid-pending` backstop this ADR establishes.
- [Design Patterns](/design-patterns.md) — the testing-strategy section this ADR's Consequences say was amended.

[^adr-0026]: ADR 0026 (docs/decisions capture, 2026-08-20)
