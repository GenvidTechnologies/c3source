...
title: "ADR 0025 — Section item-hood as a named axis, and stray files as diagnostic, not drift"
description: A new C3_SECTION_ITEM_EXTENSION/isSectionItemName/find_all_section_items_path axis makes all seven name-section finders uniformly .json-filtered (narrowing find_all_layouts_path/find_all_objectTypes_path, a breaking change), and a new detectStrayFiles reports non-item files under a section root as a diagnostic that is never drift.
tags: [adr, traversal, manifest, breaking-change]
status: stable
generated: { by: process:maintain-wiki, at: 2026-08-20T15:48:57Z }
sources:
  - id: adr-0025
    resource: ../../raw/adr-0025-section-item-hood-and-stray-files-2026-08-20.md
    title: "ADR 0025 (docs/decisions capture, 2026-08-20)"
    last_modified: 2026-08-20
---

# ADR 0025 — Section item-hood as a named axis, and stray files as diagnostic, not drift

**Status:** accepted
**Date:** 2026-08-11
**Issue:** #76

Migrated verbatim from the `docs/decisions/` ADR record[^adr-0025].
...
- [ADR 0005 — One canonical recursive walk per traversal](/decisions/0005-single-canonical-traversal-walk.md) — the shape this ADR's `find_all_section_items_path` applies, and the escape hatch (`find_all_files_path`) that keeps the narrowing non-lossy.
- [ADR 0006 — Single canonical editor-local classifier](/decisions/0006-editor-local-classifier.md) — the provenance axis of the three-axis frame this ADR names.
- [ADR 0018 — *.brush.json is minified project source, not editor-local](/decisions/0018-brush-json-minified-source-not-editor-local.md) — a sibling non-widening decision on the same provenance axis.
- [ADR 0020 — Editor-local classification does not imply walk unreachability](/decisions/0020-caller-controlled-walk-descent.md) — the reachability axis of the three-axis frame; its "residual risk to watch" is discharged by this ADR.
- [ADR 0024 — Script-source domain fact and dotted-extension convention](/decisions/0024-script-source-fact-and-dotted-extensions.md) — the sibling breaking change riding the same 2.0.0 release, and the reason `findAllScripts` is explicitly excluded from this ADR's item-hood axis.
- [Layout Traversal](/layout-traversal.md) — the current state of the three-axis classification surface, including this ADR's item-hood axis.
- [Project Manifest](/project-manifest.md#stray-files) — the current state of the `detectStrayFiles`/`ManifestDrift.strays` surface this ADR establishes.

[^adr-0025]: ADR 0025 (docs/decisions capture, 2026-08-20)
