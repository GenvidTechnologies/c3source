...
title: "ADR 0017 — Tolerant project.c3proj read mode"
description: A lenient parse path (validateProjectManifest / parseProjectManifestTolerant / readProjectManifestTolerant) returns a manifest together with a typed list of the shape violations found in it, instead of the existing strict path's all-or-nothing throw.
tags: [adr, manifest, validation]
status: stable
generated: { by: process:maintain-wiki, at: 2026-08-20T15:48:57Z }
sources:
  - id: adr-0017
    resource: ../../raw/adr-0017-tolerant-manifest-read-2026-08-20.md
    title: "ADR 0017 (docs/decisions capture, 2026-08-20)"
    last_modified: 2026-08-20
---

# ADR 0017 — Tolerant project.c3proj read mode

**Status:** accepted
**Date:** 2026-07-29
**Issue:** #58 (sibling: #57)

Migrated verbatim from the `docs/decisions/` ADR record[^adr-0017].
...
This decision depends on [ADR 0016](/decisions/0016-c3-source-json-serialization-form.md)
...
0009](/decisions/0009-editor-strict-validation.md)'s recorded split for event sheets:
"parse stays permissive for reading, validation is opt-in for write-safety."
[ADR 0011](/decisions/0011-c3-expression-tokenizer.md) is the rejected tolerance idiom this
...
This is a direct consequence of [ADR 0005](/decisions/0005-single-canonical-traversal-walk.md)
...
0016](/decisions/0016-c3-source-json-serialization-form.md)) do not call
...
- **Options bag** — `parseProjectManifest(json, opts?: {validate?: boolean})` —
  ...
  ([ADR 0011](/decisions/0011-c3-expression-tokenizer.md)): the caller gets a degraded
...
- Both alternatives were weighed directly against this repo's two existing
  ...
  0011](/decisions/0011-c3-expression-tokenizer.md)) and collected-issue-list ([ADR
  0009](/decisions/0009-editor-strict-validation.md)) — and the collected-issue-list shape
...
- A genuine follow-up is named rather than folded in here:
  ...
  `EDITOR_FIELD_RULES` ([ADR 0009](/decisions/0009-editor-strict-validation.md)), modelling
...
- [ADR 0005 — One canonical recursive walk per traversal](/decisions/0005-single-canonical-traversal-walk.md) — the pattern `collectManifestIssues` applies to shape validation.
- [ADR 0009 — Lenient parse types + editor-strictness validation layer](/decisions/0009-editor-strict-validation.md) — the event-sheet precedent this decision applies one level up the stack, and the collected-issue-list idiom this one reuses.
- [ADR 0011 — C3-expression tokenizer](/decisions/0011-c3-expression-tokenizer.md) — the silent-degradation idiom this decision deliberately avoids repeating.
- [ADR 0016 — c3source owns the C3 source-JSON write form](/decisions/0016-c3-source-json-serialization-form.md) — the write surface this decision's "read tolerantly, repair, write back" workflow depends on.
- [Project Manifest](/project-manifest.md#strict-vs-tolerant-parsing) — the current state of the strict/tolerant parsing surface this ADR establishes.

[^adr-0017]: ADR 0017 (docs/decisions capture, 2026-08-20)
