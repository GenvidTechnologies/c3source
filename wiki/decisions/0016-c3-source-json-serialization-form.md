...
title: "ADR 0016 — c3source owns the C3 source-JSON write form"
description: A new leaf module, src/serialize.ts, owns the tab-indented, no-trailing-newline C3 source-JSON write form and a write-through manifest cache rule, so downstream stops re-deriving or duplicating the invariant.
tags: [adr, serialization, manifest]
status: stable
generated: { by: process:maintain-wiki, at: 2026-08-20T15:48:57Z }
sources:
  - id: adr-0016
    resource: ../../raw/adr-0016-c3-source-json-serialization-form-2026-08-20.md
    title: "ADR 0016 (docs/decisions capture, 2026-08-20)"
    last_modified: 2026-08-20
---

# ADR 0016 — c3source owns the C3 source-JSON write form

**Status:** accepted
**Date:** 2026-07-29
**Issue:** #57 (sibling: #58)
**Amended by:** [ADR 0018](/decisions/0018-brush-json-minified-source-not-editor-local.md)
(#59) — `*.brush.json` is minified project *source*, not an editor-local file

Migrated verbatim from the `docs/decisions/` ADR record[^adr-0016].
...
category [ADR 0008](/decisions/0008-c3-domain-fact-tables.md) exists to close.
...
See [ADR 0017](/decisions/0017-tolerant-manifest-read.md) for the sibling decision this one
...
obtained *tolerantly* (see [ADR 0017](/decisions/0017-tolerant-manifest-read.md)) and wrote
...
covered separately by [ADR 0018](/decisions/0018-brush-json-minified-source-not-editor-local.md)'s
...
- **Host the serializer in `src/layouts.ts` instead of a new module** —
  ...
  *serializer* from `./layouts.js`, which reads as a mistake at every call site,
  and it deepens the exact accretion [ADR 0012](/decisions/0012-per-area-module-split.md)
...
- Neither `serializeProjectManifest` nor `writeProjectManifest` nor
  ...
  0017](/decisions/0017-tolerant-manifest-read.md) for why that is deliberate rather than
  an oversight.
...
- One documented corpus exception survives outside this decision's scope:
  ...
  0006](/decisions/0006-editor-local-classifier.md) classifier) — and never will be: the
  file is project source, not editor-local, merely written in a second
  serialization form. Resolved by [ADR
  0018](/decisions/0018-brush-json-minified-source-not-editor-local.md) (#59); the
...
- [ADR 0008 — C3 domain facts owned as exported tables](/decisions/0008-c3-domain-fact-tables.md) — the duplicated-platform-fact category this ADR closes for the write form.
- [ADR 0012 — Per-area module split](/decisions/0012-per-area-module-split.md) — the module-split rationale this decision's leaf-module placement respects.
- [ADR 0017 — Tolerant project.c3proj read mode](/decisions/0017-tolerant-manifest-read.md) — the sibling decision this ADR's write surface makes possible.
- [ADR 0018 — *.brush.json is minified project source, not editor-local](/decisions/0018-brush-json-minified-source-not-editor-local.md) — amends this ADR's Decision/Consequences to resolve the one documented round-trip exception.
- [Serialization Form](/serialization-form.md) — the current state of the write form this ADR establishes.
- [Project Manifest](/project-manifest.md) — the manifest read/write surface this ADR extends.

[^adr-0016]: ADR 0016 (docs/decisions capture, 2026-08-20)
