...
title: "ADR 0013 — Depend on fflate for .c3addon zip reading"
description: c3source adds fflate as its first runtime dependency to read real zip-form .c3addon packages, keeping a pre-read-JSON boundary so the pure ACE-model parser and attribution primitives stay testable without it.
tags: [adr, dependencies, addon-domain-layer]
status: stable
generated: { by: process:maintain-wiki, at: 2026-08-20T15:48:57Z }
sources:
  - id: adr-0013
    resource: ../../raw/adr-0013-fflate-dependency-c3addon-reader-2026-08-20.md
    title: "ADR 0013 (docs/decisions capture, 2026-08-20)"
    last_modified: 2026-08-20
---

# ADR 0013 — Depend on fflate for .c3addon zip reading

**Status:** accepted (partially revises [ADR 0001](/decisions/0001-single-module-esm-library.md)'s no-runtime-deps stance)
**Date:** 2026-07-20
**Issue:** #44

Migrated verbatim from the `docs/decisions/` ADR record[^adr-0013].
...
0001](/decisions/0001-single-module-esm-library.md) frames the library as a
...
allowlist, [ADR 0004](/decisions/0004-dist-entry-points-no-publishconfig.md), is
unchanged).
...
0001](/decisions/0001-single-module-esm-library.md) established becomes a "minimal,
flat, audited runtime deps" posture instead — this ADR partially revises
ADR 0001's no-runtime-deps stance, the same way [ADR
0012](/decisions/0012-per-area-module-split.md) notes it amends only ADR 0001's
...
- [ADR 0001 — Single-module ESM library](/decisions/0001-single-module-esm-library.md) — the no-runtime-deps stance this record partially revises.
- [ADR 0004 — Package entry points at dist/, not src/*.ts via publishConfig](/decisions/0004-dist-entry-points-no-publishconfig.md) — the files allowlist this record confirms is unchanged.
- [ADR 0012 — Per-area module split](/decisions/0012-per-area-module-split.md) — the sibling record with the same "partially revises ADR 0001" framing.
- [Addon Domain Layer](/addon-domain-layer.md) — the current addon reader, attribution, and ACE-model surface built on this dependency.

[^adr-0013]: ADR 0013 (docs/decisions capture, 2026-08-20)
