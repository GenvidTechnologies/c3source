...
title: "ADR 0023 — Pre-r402 image serialization and drift degradation"
description: deriveExpectedImages/deriveExpectedImageNames no longer throw on an absent fileType MIME (C3 before r402 emits none, matching the editor's own image/png fallback); a caught detectImageDrift throw now surfaces as ManifestDrift.degraded instead of silently vanishing.
tags: [adr, manifest, images, drift]
status: stable
generated: { by: process:maintain-wiki, at: 2026-08-20T15:48:57Z }
sources:
  - id: adr-0023
    resource: ../../raw/adr-0023-pre-r402-image-serialization-drift-degradation-2026-08-20.md
    title: "ADR 0023 (docs/decisions capture, 2026-08-20)"
    last_modified: 2026-08-20
---

# ADR 0023 — Pre-r402 image serialization and drift degradation

**Status:** accepted
**Date:** 2026-08-04
**Issue:** #68

Migrated verbatim from the `docs/decisions/` ADR record[^adr-0023]. Note per
...
- **`ManifestDrift.degraded?: DriftDegradation[]`** (`src/manifest.ts:111`,
  ...
  that *is* the caller's request ([ADR 0021](/decisions/0021-reference-integrity-detection.md)'s
  policy, applied consistently). The degrading-vs-throwing split itself is
  preserved; only its **visibility** changed.
...
- **Adding a `detector-error` to `DriftEntry.kind`** — rejected: that union
  ...
  0021](/decisions/0021-reference-integrity-detection.md) used to keep layout effects
...
- Corpus evidence for the boundary: `docs/domain-fact-audit.md`'s
  [`IMAGE_FILE_TYPE_EXTENSIONS`](/c3-domain-facts.md) section — 8,485 image
  nodes across 8 releases, 15 absent-`fileType` nodes, all in releases
  37900/39700, both pre-r402.

## Related

- [ADR 0008 — C3 domain facts owned as exported tables](/decisions/0008-c3-domain-fact-tables.md) — the domain-fact convention `IMAGE_FILE_TYPE_EXTENSIONS`/`C3_LEGACY_IMAGE_EXTENSION` belong to.
- [ADR 0021 — Reference integrity as a separate module](/decisions/0021-reference-integrity-detection.md) — the degrade-vs-throw policy this ADR applies consistently to `C3Project.detectImageDrift()`.
- [ADR 0022 — Domain-fact audit convention](/decisions/0022-domain-fact-audit-convention.md) — the audit (#68) that surfaced this table as genuinely wrong.
- [Project Manifest](/project-manifest.md#absent-vs-unmapped-filetype) — the current state of the image-drift surface this ADR establishes.
- [C3 Domain Facts](/c3-domain-facts.md) — the corpus evidence for the pre-r402 boundary.

[^adr-0023]: ADR 0023 (docs/decisions capture, 2026-08-20)
